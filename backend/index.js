// backend/index.js
// Versión: 2.6 - Creación de Posts
// Añade el endpoint POST /api/posts para crear nuevas publicaciones ("Momentos").
...
app.get('/', (req, res) => res.json({ message: '¡Bienvenido a la API de EnlaPet! v2.6 - Creación de Posts' }));
...
// --- Endpoints de Perfiles y Mascotas (sin cambios) ---
...
// -----------------------------------------------------------------------------
// [NUEVO] Endpoints de Publicaciones (Posts)
// -----------------------------------------------------------------------------
app.post('/api/posts', upload.single('postImage'), async (req, res) => {
    const { uid } = req.user;
    const { caption, authorId, authorType } = req.body; // authorType puede ser 'user' o 'pet'
    ...
    // 1. Obtener los datos del autor (sea usuario o mascota)
    let authorName = '';
    ...
    // 2. Subir la imagen del post a Firebase Storage
    const postRef = db.collection('posts').doc(); // Generamos un ID para el post
