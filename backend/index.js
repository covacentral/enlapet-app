// backend/index.js
// Versión: 2.5 - Diagnóstico Mejorado
// Añade logging detallado y manejo de errores defensivo para depurar el guardado de mascotas.
...
const allowedOrigins = new RegExp(/^https?:\/\/((www\.)?covacentral\.shop|localhost:5173|enlapet-app(-[a-z0-9-]+)?\.vercel\.app)$/);
const corsOptions = {
...
app.put('/api/profile', async (req, res) => {try{const { uid } = req.user;const { name, bio, location } = req.body;const updatedData = {};if (name !== undefined) updatedData.name = name;if (bio !== undefined) updatedData.bio = bio;if (location !== undefined) updatedData.location = location;if (Object.keys(updatedData).length === 0) return res.status(400).json({ message: 'No se proporcionaron datos para actualizar.' });await db.collection('users').doc(uid).set(updatedData, { merge: true });res.status(200).json({ message: 'Perfil actualizado con éxito.' });}catch(e){res.status(500).json({ message: 'Error interno del servidor.' })}});
// ... (endpoint de /api/profile/picture sin cambios)
...
// [MODIFICADO] Actualización de Mascota con LOGS DE DIAGNÓSTICO
app.put('/api/pets/:petId', async (req, res) => {
  const { uid } = req.user;
  const { petId } = req.params;
  const updateData = req.body;

  console.log(`[PETS_UPDATE_START] User ${uid} is updating pet ${petId}`);
  console.log('[PETS_UPDATE_DATA] Received data:', JSON.stringify(updateData, null, 2));

  try {
    if (!updateData || Object.keys(updateData).length === 0) {
      console.log('[PETS_UPDATE_FAIL] No data provided.');
    ...
    if (!petDoc.exists) {
      console.log(`[PETS_UPDATE_FAIL] Pet ${petId} not found.`);
      return res.status(404).json({ message: 'Mascota no encontrada.' });
    }
    if (petDoc.data().ownerId !== uid) {
      console.log(`[PETS_UPDATE_FAIL] User ${uid} is not the owner of pet ${petId}.`);
      return res.status(403).json({ message: 'No autorizado para modificar esta mascota.' });
    }

    console.log(`[PETS_UPDATE_FIRESTORE] Attempting to save data for pet ${petId}...`);
    await petRef.set(updateData, { merge: true });
    console.log(`[PETS_UPDATE_SUCCESS] Firestore save successful for pet ${petId}.`);

    // --- Lógica de Ubicación Implícita (más segura) ---
    try {
      if (updateData.location && updateData.location.city) {
        ...
        if (userDoc.exists) {
          ...
          if (!userData.location || !userData.location.city) {
            console.log(`[IMPLICIT_LOCATION] Updating location for user ${uid}.`);
            await userRef.set({ location: updateData.location }, { merge: true });
          }
        }
      }
    } catch (implicitLocationError) {
        // Si esto falla, solo lo registramos, pero no detenemos la respuesta exitosa principal.
        console.error('[IMPLICIT_LOCATION_ERROR] Failed to update user location implicitly:', implicitLocationError);
    }

    res.status(200).json({ message: 'Mascota actualizada con éxito.' });

  } catch (error) {
    console.error(`[PETS_UPDATE_FATAL] A critical error occurred while updating pet ${petId}:`, error);
    res.status(500).json({ message: 'Error interno del servidor al actualizar la mascota.' });
  }
});

// ... (El resto de los endpoints de mascotas sin cambios)
