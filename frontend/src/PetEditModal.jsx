...
// Versión: 3.6 - MODO VETERINARIO AÑADIDO
// TAREA: El modal ahora puede operar en un modo restringido para que los veterinarios editen solo el carné de salud.
...
function PetEditModal({ pet, user, onClose, onUpdate, isVetMode = false }) { // <-- 1. NUEVO PROP 'isVetMode'
  const [activeTab, setActiveTab] = useState('profile');
  const [formData, setFormData] = useState({
    name: '', breed: '', location: { department: '', city: '' },
  ...
  useEffect(() => {
    if (pet) {
      setFormData({
        name: pet.name || '', breed: pet.breed || '', location: pet.location || { department: '', city: '' },
        healthRecord: {
          birthDate: pet.healthRecord?.birthDate || '', gender: pet.healthRecord?.gender || '',
          vaccines: pet.healthRecord?.vaccines || [], medicalHistory: pet.healthRecord?.medicalHistory || []
        }
      });
      if (pet.location?.department) {
        setCities(colombiaData.find(d => d.departamento === pet.location.department)?.ciudades || []);
      }
      // Si es un veterinario, lo llevamos directamente al carné de salud.
      if (isVetMode) {
        setActiveTab('health');
      }
    }
  ...
  const handleChange = (e) => {
    ...
    if (section === 'healthRecord' || section === 'location') {
      ...
      if (name === 'location.department') {
        newSectionData.city = '';
        setCities(colombiaData.find(d => d.departamento === value)?.ciudades || []);
      }
  ...
  // Lógica para añadir/quitar vacunas e historial (sin cambios)
  const handleAddVaccine = (vaccine) => setFormData(prev => ({ ...prev, healthRecord: { ...prev.healthRecord, vaccines: [...prev.healthRecord.vaccines, vaccine] }}));
  const handleRemoveVaccine = (vaccineId) => setFormData(prev => ({ ...prev, healthRecord: { ...prev.healthRecord, vaccines: prev.healthRecord.vaccines.filter(v => v.id !== vaccineId) }}));
  const handleAddMedicalHistory = (entry) => setFormData(prev => ({ ...prev, healthRecord: { ...prev.healthRecord, medicalHistory: [...prev.healthRecord.medicalHistory, entry] }}));
  const handleRemoveMedicalHistory = (entryId) => setFormData(prev => ({ ...prev, healthRecord: { ...prev.healthRecord, medicalHistory: prev.healthRecord.medicalHistory.filter(h => h.id !== entryId) }}));
  ...
  const handleSaveChanges = async (e) => {
    ...
    // Si es un veterinario, nos aseguramos de enviar solo el carné de salud.
    const payload = isVetMode ? { healthRecord: formData.healthRecord } : formData;
      ...
      <div className={sharedStyles.modalBackdrop} onClick={onClose}>
        <div className={styles.content} onClick={e => e.stopPropagation()}>
          <div className={sharedStyles.modalHeader} style={{borderBottom: 'none'}}>
            <h2>{isVetMode ? `Gestionando a ${pet.name}` : `Editar Perfil de ${pet.name}`}</h2>
          ...
          <form onSubmit={handleSaveChanges}>
            <div className={sharedStyles.modalTabs} style={{padding: '0 24px'}}>
              {/* --- 2. Deshabilitamos la pestaña de perfil para veterinarios --- */}
              <button type="button" className={`${sharedStyles.modalTabButton} ${activeTab === 'profile' ? sharedStyles.active : ''}`} onClick={() => setActiveTab('profile')} disabled={isVetMode}>
                Perfil
              </button>
            ...
            <div className={styles.body}>
              {activeTab === 'profile' && (
                <>
                  ...
                  <div className={sharedStyles.formGroup}><label>Departamento:</label><select name="location.department" value={formData.location.department} onChange={handleChange} disabled={isLoading}><option value="">Selecciona un departamento</option>{departments.map(dep => <option key={dep} value={dep}>{dep}</option>)}</select></div>
                  <div className={sharedStyles.formGroup}><label>Ciudad:</label><select name="location.city" value={formData.location.city} onChange={handleChange} disabled={isLoading || !formData.location.department}><option value="">Selecciona una ciudad</option>{cities.map(city => <option key={city} value={city}>{city}</option>)}</select></div>
                </>
              )}
              ...
              {activeTab === 'health' && (
                <>
                  ...
                  <div className={styles.healthSection}>
                    <div className={styles.healthSectionHeader}><h4>Vacunas</h4>{!showAddVaccine && <button type="button" className={styles.addRecordButton} onClick={() => setShowAddVaccine(true)}><Plus size={16} /> Añadir</button>}</div>
                    {showAddVaccine && <AddVaccineForm onSave={handleAddVaccine} onCancel={() => setShowAddVaccine(false)} />}
                    <div className={styles.recordList}>{formData.healthRecord.vaccines.length > 0 ? (formData.healthRecord.vaccines.map(vaccine => (<div key={vaccine.id} className={styles.recordCard}><div className={styles.recordCardInfo}><strong>{vaccine.name}</strong><span>Aplicada: {vaccine.date} {vaccine.nextDate && `| Próxima: ${vaccine.nextDate}`}</span></div><div className={styles.recordCardActions}><button type="button" onClick={() => handleRemoveVaccine(vaccine.id)}><Trash2 size={16} /></button></div></div>))) : (!showAddVaccine && <div className={styles.emptyHealthSection}><p>Aún no has registrado ninguna vacuna.</p></div>)}</div>
                  </div>
                  <div className={styles.healthSection}>
                    <div className={styles.healthSectionHeader}><h4>Historial Clínico</h4>{!showAddHistory && <button type="button" className={styles.addRecordButton} onClick={() => setShowAddHistory(true)}><Plus size={16} /> Añadir</button>}</div>
                    {showAddHistory && <AddMedicalHistoryForm onSave={handleAddMedicalHistory} onCancel={() => setShowAddHistory(false)} />}
                    <div className={styles.recordList}>{formData.healthRecord.medicalHistory.length > 0 ? (formData.healthRecord.medicalHistory.map(entry => (<div key={entry.id} className={styles.recordCard}><div className={styles.recordCardInfo}><strong>{entry.title}</strong><span>Fecha: {entry.date}</span>{entry.description && <span style={{fontSize: '0.8rem', opacity: 0.8}}>{entry.description}</span>}</div><div className={styles.recordCardActions}><button type="button" onClick={() => handleRemoveMedicalHistory(entry.id)}><Trash2 size={16} /></button></div></div>))) : (!showAddHistory && <div className={styles.emptyHealthSection}><p>Aún no has registrado ninguna entrada en el historial.</p></div>)}</div>
                  </div>
                </>
              )}
            </div>
