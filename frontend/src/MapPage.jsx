    ...
    // Versión: 1.4 - Toggle de Eventos
    // Añade un interruptor para alternar entre la vista de lugares y eventos.
    ...
    function MapPage() {
      const [viewMode, setViewMode] = useState('locations'); // 'locations' o 'events'
      ...
      const fetchData = useCallback(async () => {
        // Al cambiar de vista, reseteamos todo para evitar inconsistencias
        setIsLoading(true);
        setError(null);
        setItems([]);
        setCategories([]);
        ...
        try {
          ...
          const itemsUrl = isLocationsView ? `${API_URL}/api/locations` : `${API_URL}/api/events`;
      ...
      }, [viewMode]); // Dependemos solo de viewMode para recargar todo
      ...
      return (
        <>
          ...
          {/* Aquí iría el modal para crear eventos */}
          ...
          <div className="map-page-container">
            ...
            <div className="map-wrapper">
              <MapContainer center={mapCenter} zoom={13} scrollWheelZoom={true} className="leaflet-container">
                ...
                {items.map(item => {
                  const position = item.coordinates ? [item.coordinates._latitude, item.coordinates._longitude] : [item.customLocation.coordinates._latitude, item.customLocation.coordinates._longitude];
                  return (
