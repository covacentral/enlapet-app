...
// (NUEVO) Perfil de rescate público y compartible para una mascota extraviada.
...
// Componente para centrar el mapa en las coordenadas una vez que cargan.
function ChangeView({ center, zoom }) {
    const map = React.useContext(React.createContext(null));
    if (map) {
...
function RescueProfile() {
    ...
    const fetchRescueProfile = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.message || 'No se pudo encontrar el perfil de rescate.');
            }
    ...
    if (isLoading) {
        return <LoadingComponent text="Cargando aviso de búsqueda..." />;
    }
    ...
    const position = [petData.lastSeen.coordinates._latitude, petData.lastSeen.coordinates._longitude];

    const WhatsAppButton = ({ phoneNumber }) => {
        if (!phoneNumber) return null;
        const cleanedPhone = phoneNumber.replace(/\D/g, '');
        // Asumimos prefijo de Colombia si no está presente
            ...
            <div className={styles.card}>
                ...
                <div className={styles.section}>
                    ...
                    <p className={styles.address}>
                        <MapPin size={16} /> {petData.lastSeen.address || 'Ubicación marcada en el mapa'}
                    </p>
                ...
                <div className={styles.section}>
                    ...
                    <p className={styles.message}>
                        <MessageSquare size={16} /> "{petData.message}"
                    </p>
                </div>
                ...
                <div className={styles.section}>
                    ...
                    {!petData.contactPhone && <p className={styles.noContact}>El dueño ha preferido no compartir su número. Por favor, llévalo a una veterinaria cercana para escanear su placa NFC.</p>}
                </div>
